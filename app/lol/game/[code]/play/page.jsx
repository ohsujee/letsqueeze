...
// Role colors for theater scenes
const ROLE_COLORS = ['#FFD700', '#06b6d4', '#ec4899', '#f59e0b', '#a78bfa', '#34d399'];

// Render a theater script with colored roles and highlighted player lines
function RenderSceneScript({ script, roles, selectedPlayers, myUid, players: allPlayers }) {
  if (!script || !roles) return <div style={{ whiteSpace: 'pre-wrap', color: 'rgba(255,255,255,0.9)', lineHeight: 1.8 }}>{script}</div>;

  // Build role→color and role→playerName maps
  const roleColorMap = {};
  const rolePlayerMap = {};
    ...
    <div>
      ...
      <div style={{ fontSize: '0.88rem', lineHeight: 1.9 }}>
        {lines.map((line, i) => {
          ...
          // Check if line starts with a role name
          ...
          if (matchedRole) {
            ...
            const afterRole = trimmed.substring(matchedRole.length + 2); // skip "ROLE : "
          ...
          // Stage direction line (starts with [)

...
const PANEL_COLORS = {
  ...
  player: { border: GAME_COLOR, labelBg: 'rgba(225,29,72,0.15)' },
...
export default function ImposteurSimulator() {
  ...
  const createRoom = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/api/dev/seed-players', {
        ...
        body: JSON.stringify({ roomCode: code, prefix: ROOM_PREFIX, action: 'seed', count: 5 }),
      });
  ...
  // ── Render panels ──
  const renderPanels = () => {
    const gamePhases = ['roles', 'describing', 'voting', 'elimination', 'roundEnd'];

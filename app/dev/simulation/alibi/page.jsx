...
import { AlibiPrepContent } from '@/app/alibi/game/[code]/prep/page';
import { AlibiPlayContent } from '@/app/alibi/game/[code]/play/page';
import { AlibiEndContent } from '@/app/alibi/game/[code]/end/page';
...
const PHASE_TABS = [
  ...
  { id: 'prep', label: 'Prépa', color: '#8b5cf6' },
  { id: 'interrogation', label: 'Interrogation', color: '#22c55e' },
...
export default function AlibiSimulator() {
  ...
  const createRoom = useCallback(async () => {
    ...
    try {
      ...
      const { writePromise } = await createFirebaseRoom({
        gameId: 'alibi', code, hostUid: myUid, hostName: 'Host (Sim)', db, ref, set,
      });
      await writePromise;

      await set(ref(db, `${ROOM_PREFIX}/${code}/players/${myUid}`), {
        uid: myUid, name: 'Host (You)', score: 0, team: 'suspects',
  ...
  const addPlayer = useCallback(async () => {
    ...
    try {
      await set(ref(db, `${ROOM_PREFIX}/${roomCode}/players/${uid}`), {
        uid, name, score: 0, team: num % 2 === 0 ? 'inspectors' : 'suspects',
    ...
    return (
      <div style={{
        ...
        <p style={{
          ...
          Crée une room Firebase réelle avec 6 joueurs (1 host + 5 faux) et visualise toutes les vues côte à côte.
        </p>
  ...
  // ===== SIMULATION VIEW =====
  const renderPanels = () => {
    if (displayPhase === 'prep') {
      return playerEntries.map(([uid, player]) => (
        <SimPanel key={uid} role={uid === myUid ? 'host' : 'player'} label={`Prep - ${player.name || uid}${uid === myUid ? ' (You)' : ''}`}>
          <AlibiPrepContent code={roomCode} myUid={uid} />
        </SimPanel>
      ));
    }

    if (displayPhase === 'interrogation') {
      return playerEntries.map(([uid, player]) => (
        <SimPanel key={uid} role={uid === myUid ? 'host' : 'player'} label={`Play - ${player.name || uid}${uid === myUid ? ' (You)' : ''}`}>
          <AlibiPlayContent code={roomCode} myUid={uid} />
        </SimPanel>
      ));
    }
    ...
    if (displayPhase === 'ended') {
      return playerEntries.map(([uid, player]) => (
        <SimPanel key={uid} role="end" label={`End - ${player.name || uid}${uid === myUid ? ' (You)' : ''}`}>
          <AlibiEndContent code={roomCode} myUid={uid} />
        </SimPanel>
      ));
    }

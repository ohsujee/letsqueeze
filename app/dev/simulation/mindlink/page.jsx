...
function PhaseLoader() {
  return <div style={{ flex: 1, display: 'flex', background: '#ec4899' }} />;
}
      ...
      return (
        <>
          ...
          {defenders.map(([uid, player]) => (
            <SimPanel key={uid} role="defender" label={`Defender - ${player.name || uid}${uid === myUid ? ' (You)' : ''}`} phoneW={device.w} phoneH={device.h}>
              <Suspense fallback={<PhaseLoader />}><MindLinkDefendContent code={roomCode} myUid={uid} /></Suspense>
            </SimPanel>
          ))}
          {/* Attacker panels */}
          {attackers.map(([uid, player]) => (
            <SimPanel key={uid} role={uid === myUid ? 'host' : 'attacker'} label={`Attacker - ${player.name || uid}${uid === myUid ? ' (You)' : ''}`} phoneW={device.w} phoneH={device.h}>
    ...
    return (
      <>
        ...
        {playerEntries.filter(([uid]) => uid !== myUid).map(([uid, player]) => (
          <SimPanel key={uid} role="attacker" label={`Player - ${player.name || uid}`} phoneW={device.w} phoneH={device.h}>
        ...
        <div style={{ display: 'flex', gap: '2px', padding: '2px', background: 'rgba(238,242,255,0.04)', borderRadius: '6px', border: '1px solid rgba(238,242,255,0.08)' }}>
          {DEVICE_PRESETS.map(d => (
            <button key={d.id} onClick={() => setDevice(d)} style={{
              ...
              background: device.id === d.id ? 'rgba(236,72,153,0.3)' : 'transparent',
              color: device.id === d.id ? '#ec4899' : 'rgba(238,242,255,0.3)',

...
import { MimeLobbyContent } from '@/app/mime/room/[code]/page';
import { MimePlayContent } from '@/app/mime/game/[code]/play/page';
import { MimeEndContent } from '@/app/mime/game/[code]/end/page';
...
export default function MimeSimulator() {
  ...
  const createRoom = useCallback(async () => {
    ...
    try {
      ...
      const { writePromise } = await createFirebaseRoom({
        gameId: 'mime', code, hostUid: myUid, hostName: 'Host (Sim)', db, ref, set,
      });
      await writePromise;

      await set(ref(db, `${ROOM_PREFIX}/${code}/players/${myUid}`), {
        uid: myUid, name: 'Host (You)', score: 0, blockedUntil: 0,
        status: 'active', joinedAt: Date.now(), location: 'lobby',
      });
  ...
  const resetToLobby = useCallback(async () => {
    ...
    Object.keys(players).forEach(uid => {
      ...
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/location`] = 'lobby';
    });
    updates[`${ROOM_PREFIX}/${roomCode}/words`] = null;
  ...
  const addPlayer = useCallback(async () => {
    ...
    try {
      await set(ref(db, `${ROOM_PREFIX}/${roomCode}/players/${uid}`), {
        uid, name, score: 0, blockedUntil: 0,
        status: 'active', joinedAt: Date.now(), isFake: true, location: 'lobby',
      });
    ...
    if (displayPhase === 'playing') {
      return playerEntries.map(([uid, player]) => (
        <SimPanel key={uid} role={uid === myUid ? 'host' : 'player'} label={`Play - ${player.name || uid}${uid === myUid ? ' (You)' : ''}`}>
          <MimePlayContent code={roomCode} myUid={uid} />
        </SimPanel>
      ));
    }
    ...
    if (displayPhase === 'ended') {
      return playerEntries.map(([uid, player]) => (
        <SimPanel key={uid} role="end" label={`End - ${player.name || uid}${uid === myUid ? ' (You)' : ''}`}>
          <MimeEndContent code={roomCode} myUid={uid} />
        </SimPanel>
      ));
    }

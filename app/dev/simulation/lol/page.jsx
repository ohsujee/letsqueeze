...
const PANEL_COLORS = {
  ...
  player: { border: '#EF4444', labelBg: 'rgba(239,68,68,0.15)' },
...
function PhaseLoader() {
  return <div style={{ flex: 1, display: 'flex', background: '#EF4444' }} />;
}

export default function LolSimulator() {
  ...
  const resetToLobby = useCallback(async () => {
    ...
    Object.keys(players).forEach(uid => {
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/yellowCards`] = 0;
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/redCard`] = false;
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/jokersRemaining`] = 1;
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/status`] = 'active';
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/accusationsMade`] = 0;
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/accusationsReceived`] = 0;
      updates[`${ROOM_PREFIX}/${roomCode}/players/${uid}/jokersPlayed`] = 0;
    });
    ...
    return (
      <div style={{
        ...
        <button
          ...
          style={{
            padding: '14px 32px',
            background: creating ? 'rgba(239,68,68,0.3)' : '#EF4444',
        ...
        {!myUid && (
          <p style={{
            ...
            Not authenticated. <a href="/dev/signin" style={{ color: '#EF4444' }}>Sign in first</a>
          </p>
        )}
      </div>
    );
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          ROOM <span style={{ color: '#EF4444', fontWeight: 700 }}>{roomCode}</span>
        </div>
      </div>

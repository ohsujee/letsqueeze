...
function RolesSection({ content }) {
  ...
  return (
    <div className="section-roles">
      {items.map((role, i) => (
        <motion.div key={role.name} className="role-card" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} style={{ borderColor: `${role.color}40` }}>
        ...
        {content.items.map((item, i) => {
          ...
          return (
            <motion.div key={item.label} className="scoring-item" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}>
              <div className="scoring-icon" style={{ background: `${item.color}20`, color: item.color }}><ItemIcon size={20} /></div>
        ...
        return (
          <motion.div key={phase.name} className="phase-card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} style={{ borderLeftColor: phase.color }}>
            <div className="phase-header">
              <div className="phase-icon" style={{ background: `${phase.color}20`, color: phase.color }}><PhaseIcon size={20} /></div>
  ...
  return (
    <div className="section-verdict-inline">
      {content.outcomes.map((outcome, i) => {
        ...
        return (
          <motion.div key={outcome.title} className="verdict-inline-card" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} style={{ borderColor: `${outcome.color}40` }}>
            <div className="verdict-inline-icon" style={{ background: `${outcome.color}20`, color: outcome.color }}><OutcomeIcon size={22} /></div>
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div className="htp-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
          ...
          <style jsx global>{`
            .htp-overlay { position:fixed; inset:0; z-index:9999; background:rgba(0,0,0,0.9); backdrop-filter:blur(10px); display:flex; align-items:center; justify-content:center; padding:16px; }
            .htp-modal { width:100%; max-width:480px; height:85vh; max-height:700px; background:linear-gradient(180deg,rgba(25,25,35,0.98) 0%,rgba(15,15,22,0.99) 100%); border:1px solid rgba(255,255,255,0.1); border-radius:24px; overflow:hidden; display:flex; flex-direction:column; box-shadow:0 25px 80px rgba(0,0,0,0.5),0 0 60px var(--accent-glow),inset 0 1px 0 rgba(255,255,255,0.05); }
            .htp-header { position:relative; padding:24px 24px 16px; text-align:center; background:var(--accent-gradient); }
            .htp-close { position:absolute; top:12px; right:12px; width:36px; height:36px; border-radius:50%; background:rgba(0,0,0,0.3); border:none; color:white; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all 0.2s; }
            .htp-close:hover { background:rgba(0,0,0,0.5); transform:scale(1.1); }
            .htp-title { font-family:'Bungee',cursive; font-size:1.75rem; color:white; margin:0; text-transform:uppercase; text-shadow:0 2px 10px rgba(0,0,0,0.3); }
            .htp-subtitle { font-family:'Inter',sans-serif; font-size:0.9375rem; color:rgba(255,255,255,0.8); margin:6px 0 0; }
            .htp-content { flex:1; overflow-y:auto; padding:20px 24px; }
            .htp-section-title { display:inline-flex; align-items:center; gap:8px; font-family:'Space Grotesk',sans-serif; font-size:0.75rem; font-weight:700; text-transform:uppercase; letter-spacing:0.1em; color:var(--accent); margin:0 0 16px; padding:6px 12px 6px 10px; background:linear-gradient(135deg,rgba(255,255,255,0.08) 0%,rgba(255,255,255,0.03) 100%); border-left:3px solid var(--accent); border-radius:0 8px 8px 0; }
            .section-intro .intro-text { font-family:'Inter',sans-serif; font-size:1rem; line-height:1.6; color:rgba(255,255,255,0.85); margin:0; }
            .section-roles { display:flex; flex-direction:column; gap:12px; }
            .role-card { padding:16px; background:rgba(255,255,255,0.03); border:1px solid; border-radius:14px; }
            ...
            .role-description { font-family:'Inter',sans-serif; font-size:0.875rem; line-height:1.5; color:rgba(255,255,255,0.7); margin:0; }
            .section-steps { display:flex; flex-direction:column; gap:16px; }
            ...
            .step-title { font-family:'Space Grotesk',sans-serif; font-size:0.9375rem; font-weight:600; color:white; margin:0 0 2px; }
            .step-description { font-family:'Inter',sans-serif; font-size:0.8125rem; color:rgba(255,255,255,0.6); margin:0; }
            .section-timeline-simple .timeline-description { font-family:'Inter',sans-serif; font-size:1rem; line-height:1.6; color:rgba(255,255,255,0.85); margin:0 0 16px; }
            .section-timeline-simple .timeline-note { display:flex; align-items:flex-start; gap:8px; font-family:'Inter',sans-serif; font-size:0.875rem; line-height:1.5; color:rgba(255,255,255,0.6); margin:0; padding:12px; background:rgba(255,255,255,0.03); border-radius:10px; }
            .section-timeline-simple .timeline-note svg { flex-shrink:0; margin-top:2px; color:var(--accent); }
            .scoring-items { display:flex; flex-direction:column; gap:10px; margin-bottom:12px; }
            .scoring-item { display:flex; align-items:center; gap:12px; padding:12px 14px; background:rgba(255,255,255,0.03); border-radius:12px; }
            .scoring-icon { width:36px; height:36px; border-radius:10px; display:flex; align-items:center; justify-content:center; }
            .scoring-label { flex:1; font-family:'Inter',sans-serif; font-size:0.9375rem; color:rgba(255,255,255,0.85); }
            .scoring-value { font-family:'Space Grotesk',sans-serif; font-size:1rem; font-weight:700; }
            .scoring-note { font-family:'Inter',sans-serif; font-size:0.8125rem; color:rgba(255,255,255,0.5); margin:0; font-style:italic; }
            .section-phases { display:flex; flex-direction:column; gap:12px; }
            .phase-card { padding:16px; background:rgba(255,255,255,0.03); border-left:3px solid; border-radius:0 12px 12px 0; }
            ...
            .phase-description { font-family:'Inter',sans-serif; font-size:0.875rem; line-height:1.5; color:rgba(255,255,255,0.7); margin:0; }
            .section-verdict-inline { display:flex; flex-direction:column; gap:12px; }
            .verdict-inline-card { display:flex; align-items:center; gap:14px; padding:16px; background:rgba(255,255,255,0.03); border:1px solid; border-radius:14px; }
            ...
            .verdict-inline-condition { font-family:'Inter',sans-serif; font-size:0.8125rem; line-height:1.4; color:rgba(255,255,255,0.6); margin:0; }
            .htp-footer { display:flex; flex-direction:column; gap:12px; padding:16px 24px 24px; border-top:1px solid rgba(255,255,255,0.05); }
            .htp-footer-actions { display:flex; gap:12px; }
            .htp-btn-prev, .htp-btn-next, .htp-btn-done { flex:1; padding:14px 20px; border-radius:12px; font-family:'Space Grotesk',sans-serif; font-size:0.9375rem; font-weight:600; cursor:pointer; transition:all 0.2s; display:flex; align-items:center; justify-content:center; gap:6px; }
            .htp-btn-prev { background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); color:rgba(255,255,255,0.6); }
            .htp-btn-prev:hover:not(:disabled) { background:rgba(255,255,255,0.1); color:white; }
            .htp-btn-prev:disabled { opacity:0.3; cursor:not-allowed; }
            .htp-btn-next { background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.15); color:white; }
            .htp-btn-next:hover { background:rgba(255,255,255,0.15); }
            .htp-btn-done { background:var(--accent-gradient); border:none; color:white; box-shadow:0 4px 20px var(--accent-glow); }
            .htp-btn-done:hover { transform:translateY(-2px); box-shadow:0 6px 25px var(--accent-glow); }
            .htp-dismiss-check { display:flex; align-items:center; gap:10px; cursor:pointer; user-select:none; }
            .htp-dismiss-check input[type="checkbox"] { appearance:none; -webkit-appearance:none; width:18px; height:18px; border:2px solid rgba(255,255,255,0.25); border-radius:4px; background:transparent; cursor:pointer; flex-shrink:0; transition:border-color 0.15s,background 0.15s; position:relative; }
            .htp-dismiss-check input[type="checkbox"]:checked { background:rgba(255,255,255,0.15); border-color:rgba(255,255,255,0.6); }
            ...
            .htp-dismiss-check span { font-family:'Inter',sans-serif; font-size:0.8125rem; color:rgba(255,255,255,0.45); }
          `}</style>
        </motion.div>
      )}
    </AnimatePresence>

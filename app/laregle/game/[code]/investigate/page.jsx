...
import GuessVoteSheet from "./_components/GuessVoteSheet";
import { ChoosingWaitPhase, PlayingInvestPhase, GuessingInvestPhase } from "./_components/InvestigatePhases";
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: '#04060f', position: 'relative' }}>
      ...
      <main style={{
        ...
        <div style={{ maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          {state.phase === 'choosing' && <ChoosingWaitPhase />}

          {/* PHASE: PLAYING */}
          {state.phase === 'playing' && (
            <PlayingInvestPhase state={state} gamePlayers={gamePlayers} eliminations={eliminations} flashUid={flashUid} />
          )}

          {/* PHASE: GUESSING */}
          {state.phase === 'guessing' && (
            <GuessingInvestPhase state={state} gamePlayers={gamePlayers} />
          )}
        </div>
      </main>
      ...
      {/* Guess Modal */}
      <GuessVoteSheet state={state} gamePlayers={gamePlayers} handleCancelGuess={handleCancelGuess} />

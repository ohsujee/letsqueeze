...
import SemanticLeaderboard from '../semantique/SemanticLeaderboard';
import LeaderboardErrorBoundary from '@/components/shared/LeaderboardErrorBoundary';
import '../semantique/semantique.css';
import {
...
} from '../semantique/SemanticComponents';
// ─── Page principale ──────────────────────────────────────────────────────────
export default function SemantiquePage() {
  ...
  const { todayState, todayDate, streak, stats, progress, startGame, saveProgress, completeGame, writeLeaderboard, loaded } =
    useDailyGame('semantique-v2', { forceDate: serverDate });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/daily/semantic-v2-score?date=${todayDate}&word=${encodeURIComponent(raw)}`);
  ...
  const handlePlayAlternative = useCallback(async () => {
    ...
    try {
      const uid = auth.currentUser?.uid;
      const res = await fetch(`/api/daily/semantic-v2-alternative?date=${todayDate}${uid ? `&uid=${uid}` : ''}`);
  ...
  const handleAltSubmit = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/daily/semantic-v2-alternative-score', {
        method: 'POST',

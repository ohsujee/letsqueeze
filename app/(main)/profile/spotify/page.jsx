...
import { useServiceConnection } from '@/lib/hooks/useServiceConnection';
...
export default function SpotifySettingsPage() {
  ...
  // Load Spotify user data when connected
  const loadSpotifyUserData = useCallback(async () => {
    try {
  ...
  const {
    ...
    checkFn: isSpotifyConnected,
    connectFn: startSpotifyAuth,
    disconnectFn: () => {
      clearTokens();
      setSpotifyUser(null);
      setIsPremium(false);
    },
    onConnected: loadSpotifyUserData,
    autoCheck: !!user, // Only check when user is loaded
    enabled: !!user,
  });

  if (authLoading) {
    return <LoadingScreen game="blindtest" />;
  }

...
import { SkipForward, X, Check, Music, Play, Pause, Bell, RefreshCw, Shuffle, Trophy } from "lucide-react";
  ...
  return (
    <div className="deeztest-host-page game-page">
          ...
          >
            ...
            >
              ...
              {revealWinner && (
                ...
                >
                  ...
                  <div className="reveal-winner-info">
                    <span className="reveal-winner-name">{revealWinner.name}</span>
                    <span className="reveal-winner-detail">a trouvé en {revealWinner.level}</span>
                  </div>
                  <div className="reveal-winner-points-box">
                    <span className="reveal-winner-points-value">+{revealWinner.points}</span>
              ...
              >
                ...
                {revealTrack.albumArt ? (
                  <img src={revealTrack.albumArt} alt="Album" className="reveal-album-art" />
                ) : (
              ...
              >
                <h2 className="reveal-track-title">{revealTrack.title}</h2>
                <p className="reveal-track-artist">{revealTrack.artist}</p>
              </motion.div>
              ...
              >
                <div
                  ref={revealProgressBarRef}
                  className={`reveal-progress-bar ${isRevealDragging ? 'dragging' : ''}`}
                  onMouseDown={handleRevealDragStart}
                  onTouchStart={handleRevealDragStart}
                >
                  <div
                ...
                <div className="reveal-progress-time">
                  <span>{Math.floor((revealProgress / 100) * REVEAL_DURATION)}s</span>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
              >
                {isRevealPlaying ? <Pause size={28} /> : <Play size={28} fill="white" style={{ marginLeft: 3 }} />}
              </motion.button>

              {/* Next Button */}
              <motion.button
                className="reveal-next-btn"
                onClick={closeRevealAndNext}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
      ...
      <style jsx>{`
        ...
        :global(.reveal-bg-effects) {
          position: absolute;
          inset: 0;
          overflow: hidden;
        }

        :global(.reveal-bg-gradient) {
          position: absolute;
          inset: 0;
          background:
        ...
        :global(.reveal-winner-info) {
          display: flex;
          flex-direction: column;
          gap: 2px;
        }

        :global(.reveal-winner-name) {
          font-family: var(--font-title, 'Bungee'), cursive;
          font-size: 1rem;
          color: white;
        }

        :global(.reveal-winner-detail) {
          font-family: var(--font-display, 'Space Grotesk'), sans-serif;
          font-size: 0.75rem;
          color: rgba(255, 255, 255, 0.6);
        }

        :global(.reveal-winner-points-box) {
          display: flex;
          flex-direction: column;
          align-items: center;
          padding: 8px 14px;
        ...
        :global(.reveal-winner-points-value) {
          font-family: var(--font-title, 'Bungee'), cursive;
          font-size: 1.1rem;
          color: #4ade80;
          line-height: 1;
        ...
        :global(.reveal-winner-points-label) {
          font-size: 0.65rem;
        ...
        /* Album Art */
        :global(.reveal-album-container) {
          position: relative;
          width: 200px;
          height: 200px;
        }

        :global(.reveal-album-glow) {
          position: absolute;
          inset: -20px;
          background: linear-gradient(135deg, ${DEEZER_PURPLE}, ${DEEZER_PINK});
          border-radius: 24px;
        ...
        :global(.reveal-album-placeholder) {
          position: relative;
          width: 100%;
          height: 100%;
          border-radius: 16px;
        ...
        :global(.reveal-playing-indicator) {
          position: absolute;
          bottom: -12px;
          left: 50%;
          transform: translateX(-50%);
        ...
        :global(.reveal-playing-indicator span) {
          width: 4px;
          background: white;
          border-radius: 2px;
        ...
        :global(.reveal-playing-indicator span:nth-child(1)) { height: 8px; animation-delay: 0s; }
        :global(.reveal-playing-indicator span:nth-child(2)) { height: 14px; animation-delay: 0.15s; }
        :global(.reveal-playing-indicator span:nth-child(3)) { height: 10px; animation-delay: 0.3s; }
        :global(.reveal-playing-indicator span:nth-child(4)) { height: 6px; animation-delay: 0.45s; }
        ...
        /* Track Info */
        :global(.reveal-track-info) {
          text-align: center;
        }

        :global(.reveal-track-title) {
          font-family: var(--font-title, 'Bungee'), cursive;
          font-size: 1.5rem;
          margin: 0 0 8px 0;
          color: white;
        ...
        /* Progress Bar */
        :global(.reveal-progress-container) {
          width: 100%;
          display: flex;
          flex-direction: column;
          gap: 8px;
        ...
        /* No transition - both fill and thumb move together instantly */

        :global(.reveal-progress-thumb) {
          position: absolute;
          top: 50%;
          width: 12px;
          height: 20px;
        ...
        :global(.reveal-progress-bar:hover .reveal-progress-thumb),
        :global(.reveal-progress-bar.dragging .reveal-progress-thumb) {
          transform: translate(-50%, -50%) scale(1.1);
          box-shadow: 0 2px 12px rgba(0, 0, 0, 0.4);
        }

        :global(.reveal-progress-bar.dragging) {
          cursor: grabbing;
        }

        :global(.reveal-progress-time) {
          display: flex;
          justify-content: space-between;
          font-size: 0.75rem;
        ...
        :global(.reveal-play-pause-btn:hover) {
        ...
        :global(.reveal-play-pause-btn:active) {
          transform: scale(0.95);
        }

        /* Next Button */
        :global(.reveal-next-btn) {
          width: 100%;
          display: flex;
          align-items: center;
          justify-content: center;
        ...
        :global(.reveal-next-btn:hover) {
          background: rgba(255, 255, 255, 0.15);
          border-color: rgba(255, 255, 255, 0.3);
          transform: translateY(-2px);
        }

        :global(.reveal-next-btn:active) {

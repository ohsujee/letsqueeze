...
export function useGameLimits(gameType, isPro = false) {
  ...
  // 3 parties gratuites + 3 parties bonus via pub
  const limits = {
    freeGames: 3,
    rewardedGames: 3
  };
  ...
  const rewardedGamesRemaining = Math.max(0, limits.rewardedGames - rewardedUsed);
  const totalGamesRemaining = freeGamesRemaining + rewardedGamesRemaining;

  // États
  const canPlayFree = isPro || freeGamesRemaining > 0;
  const canWatchAdForGame = !isPro && freeGamesRemaining === 0 && rewardedGamesRemaining > 0;
  const isBlocked = !isPro && totalGamesRemaining === 0;
  ...
  const watchAdForExtraGame = useCallback(async () => {
    if (isPro || rewardedGamesRemaining <= 0) {
      return false;
    }
  ...
  }, [gameType, isPro, rewardedGamesRemaining]);

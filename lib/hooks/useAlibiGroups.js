...
export function useAlibiGroups({
  ...
  // Liste des IDs de groupes actifs (fallback sur groupCount si Firebase pas encore arrivé)
  const groupIds = useMemo(() => {
    const fromFirebase = Object.keys(groups).filter(id => id.startsWith('group'));
    if (fromFirebase.length > 0) return fromFirebase;
    // Fallback: générer les IDs depuis meta.groupCount
    return Array.from({ length: groupCount }, (_, i) => `group${i + 1}`);
  }, [groups, groupCount]);
  ...
  const initializeGroups = useCallback(async (count) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      newGroups[groupId] = {
        ...
        color: ALIBI_GROUP_CONFIG.DEFAULT_COLORS[i % ALIBI_GROUP_CONFIG.DEFAULT_COLORS.length],
        alibiId: null,
        alibiData: null,

...
export function useForceUpdate() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Écoute le nœud config/ entier pour lire la version par plateforme
      // Priorité : forceUpdateVersionIos/Android > forceUpdateVersion (global)
      const platform = Capacitor.getPlatform(); // 'ios' | 'android'
      const platformKey = platform === 'ios' ? 'forceUpdateVersionIos' : 'forceUpdateVersionAndroid';

      const configRef = ref(db, 'config');
      unsubscribe = onValue(configRef, (snap) => {
        const config = snap.val() || {};
        // Utilise la version spécifique à la plateforme si elle existe, sinon le global
        const minVersion = config[platformKey] || config.forceUpdateVersion;

 ...
 * @param {string} [options.context] - Nom du contexte pour les logs (ex: 'Room', 'BlindTest', 'Mime')
...
export function useInterstitialAd(options = {}) {
  ...
  useEffect(() => {
    ...
    // Check 1: Retour d'une partie (ne pas montrer de pub)
    ...
    // Check 2: Pub déjà montrée pendant le join flow
    const adShownDuringJoin = storage.get('adShownDuringJoin');
    if (adShownDuringJoin) {
      storage.remove('adShownDuringJoin');
      adShownRef.current = true;
      return;
    }
...
/**
 * Fonction utilitaire pour marquer qu'une pub a été montrée pendant le join
 * (utilisé par JoinClient avant de naviguer vers la room)
 */
export function markAdShownDuringJoin() {
  storage.set('adShownDuringJoin', true);
}

/**
 * Vérifie si on doit montrer une pub (version synchrone pour JoinClient)
 * @param {boolean} userIsPro
 * @returns {boolean}
 */
export function shouldShowInterstitial(userIsPro) {
  if (userIsPro) return false;

  const returnedFromGame = storage.get('returnedFromGame');
  if (returnedFromGame) return false;

...
export function useRoomGuard({
  ...
  // Fonction pour fermer la room (hôte uniquement) — marque comme fermée
      ...
      if (!isHost && meta.hostDisconnectedAt) {
        ...
        if (elapsed >= HOST_GRACE_PERIOD_MS) {
          ...
          gracePeriodTimerRef.current = setTimeout(() => {
            // Grace period expired — force a re-read of meta to trigger redirect
            // The onValue listener may not re-fire if data hasn't changed,
            // so we read once to re-evaluate
            get(metaRef).then((snap) => {
              const freshMeta = snap.val();
              if (freshMeta?.hostDisconnectedAt && !hasRedirectedRef.current && !isLeavingVoluntarilyRef.current) {
                const freshElapsed = Date.now() - freshMeta.hostDisconnectedAt;
                if (freshElapsed >= HOST_GRACE_PERIOD_MS) {
                  hasRedirectedRef.current = true;
                  storage.remove('last_game');
                  toast.warning("L'hôte a quitté la partie");
                  router.push('/home');

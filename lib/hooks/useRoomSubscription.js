 ...
 * @param {string} roomPrefix - Le préfixe Firebase de la room (ex: "rooms", "rooms_alibi")
 ...
 */
export function useRoomSubscription(code, roomPrefix = "rooms", options = {}) {
  ...
  useEffect(() => {
    ...
    const metaUnsub = onValue(ref(db, `${roomPrefix}/${code}/meta`), (snap) => {
      const m = snap.val();
      if (m) {
    ...
    // Players listener
    const playersUnsub = onValue(ref(db, `${roomPrefix}/${code}/players`), (snap) => {
      const p = snap.val() || {};
    ...
    let stateUnsub = () => {};
    if (onStateUpdate) {
      stateUnsub = onValue(ref(db, `${roomPrefix}/${code}/state`), (snap) => {
  ...
  }, [code, roomPrefix, router, toast, onMetaUpdate, onPlayersUpdate, onStateUpdate]);

 ...
 * - Force reconnexion via re-register presence
 *
 * Structure Firebase:
...
export function usePresence({
  ...
  // Re-registers presence without using goOffline/goOnline which would
  // disconnect ALL Firebase listeners globally (causing false disconnects in other hooks)
  ...
  }, [enabled, registerPresence, sendHeartbeat]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(connectedRef, (snapshot) => {
      ...
      if (connected) {
        // Connecté - enregistrer la présence (fire-and-forget with catch)
        registerPresence().catch((err) => {
          console.error('[usePresence] registerPresence failed:', err);
        });
      } else {

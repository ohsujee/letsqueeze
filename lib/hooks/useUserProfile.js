...
const FOUNDER_CACHE_KEY = 'lq_is_founder';
...
// Read cached founder status synchronously (available before Firebase loads)
const getCachedIsFounder = () => {
  if (typeof window === 'undefined') return false;
  return localStorage.getItem(FOUNDER_CACHE_KEY) === 'true';
};

// Update founder cache when status changes
const setCachedIsFounder = (value) => {
  if (typeof window === 'undefined') return;
  if (value) {
    localStorage.setItem(FOUNDER_CACHE_KEY, 'true');
  } else {
    localStorage.removeItem(FOUNDER_CACHE_KEY);
  }
};
...
export function useUserProfile() {
  ...
  const [isFounder, setIsFounder] = useState(() => getCachedIsFounder());
        ...
        try {
          ...
          unsubscribeUserData = onValue(userRef, (snapshot) => {
            ...
            if (data) {
              ...
              const founderStatus = data.isFounder === true;
              setIsFounder(founderStatus);
              setCachedIsFounder(founderStatus);
  ...
  return {
    ...
    // Helper values
    isLoggedIn: !!user,
    isPro: subscription?.tier === 'pro' || isFounder,
    level: stats?.level || 1,
    xp: stats?.xp || 0,

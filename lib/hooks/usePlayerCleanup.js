...
export function usePlayerCleanup({
  ...
  const isPlayerMissingRef = useRef(false);
  ...
  const setupDisconnectHandler = useCallback(async () => {
    ...
    if (phase === 'lobby' || phase === 'playing') {
      ...
      try {
        ...
        console.error('[PlayerCleanup] onDisconnect setup failed:', e);
      }
    }
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(playerRef, async (snapshot) => {
      ...
      if (!exists && !isPlayerMissingRef.current) {
        ...
        try {
          ...
          if (kickedSnap.exists()) {
            ...
            isPlayerMissingRef.current = true;
      ...
      } else if (exists && isPlayerMissingRef.current) {
        // Player was re-added (successful rejoin)
        isPlayerMissingRef.current = false;
        setIsPlayerMissing(false);
      }
    });

    return () => unsubscribe();
  }, [roomCode, roomPrefix, playerUid, phase, getCode, onPlayerRemoved]);
...
// Game type info for RejoinBanner (WebP optimisées)
const GAME_TYPES = {
  rooms: { name: 'Quiz Buzzer', image: '/images/optimized/quiz-buzzer.webp', progressKey: 'currentIndex', progressLabel: 'Question' },
  rooms_blindtest: { name: 'DeezTest', image: '/images/optimized/blind-test.webp', progressKey: 'currentIndex', progressLabel: 'Piste' },
  rooms_alibi: { name: 'Alibi', image: '/images/optimized/alibi.webp', progressKey: 'currentQuestion', progressLabel: 'Question', totalFixed: 10 },
  rooms_laregle: { name: 'La Règle', image: '/images/optimized/laregle.webp', progressKey: 'roundNumber', progressLabel: 'Manche' }
};

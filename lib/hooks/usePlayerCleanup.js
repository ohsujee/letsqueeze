...
const GAME_TYPES = {
  ...
  rooms_deeztest: { name: 'Blind Test', image: '/images/blind-test.png', progressKey: 'currentIndex', progressLabel: 'Piste' },
  rooms_alibi: { name: 'Alibi', image: '/images/alibi.png', progressKey: 'currentQuestion', progressLabel: 'Question', totalFixed: 10 },
  rooms_trouveregle: { name: 'Trouve la Règle', image: '/images/trouve-la-regle.png', progressKey: 'roundNumber', progressLabel: 'Manche' }
};
        ...
        if (state) {
          ...
          if (playerData && (playerData.status === 'disconnected' || playerData.status === 'left')) {
            ...
            if (roomPrefix === 'rooms_blindtest') {
              ...
              rejoinUrl = isLobby ? `/deeztest/room/${roomCode}` : `/deeztest/game/${roomCode}/play`;
              ...
              rejoinUrl = isLobby ? `/trouveregle/room/${roomCode}` : `/trouveregle/game/${roomCode}/play`;
            } else {
            ...
            if (!totalProgress) {
              try {
                if (roomPrefix === 'rooms_blindtest' || roomPrefix === 'rooms_deeztest') {
                  // BlindTest/DeezTest: total in tracks array

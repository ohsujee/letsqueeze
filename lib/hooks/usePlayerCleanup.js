...
export function usePlayerCleanup({ roomCode, roomPrefix = 'rooms', playerUid, phase }) {
  ...
  useEffect(() => {
    ...
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'hidden') {
        ...
        // Re-setup disconnect handler in case connection was lost
        setupDisconnectHandler();
      }
    };
  ...
  }, [roomCode, playerUid, setupDisconnectHandler]);
          ...
          onValue(playerRef, (playerSnap) => {
            ...
            if (playerData && (playerData.status === 'disconnected' || playerData.status === 'left')) {
              setActiveGame({
                ...
                canRejoin: true,
                rejoinUrl: roomPrefix === 'rooms_blindtest'
                  ? `/blindtest/game/${roomCode}/play`
                  : `/game/${roomCode}/play`
              });
            } else {
              setActiveGame(null);

...
const GAME_TYPES = {
  ...
  rooms_laloi: { name: 'La Loi', image: '/images/laloi.png', progressKey: 'roundNumber', progressLabel: 'Manche' }
};
      ...
      try {
        ...
        if (state) {
          ...
          if (playerData && (playerData.status === 'disconnected' || playerData.status === 'left')) {
            ...
            } else if (roomPrefix === 'rooms_laloi') {
              rejoinUrl = isLobby ? `/laloi/room/${roomCode}` : `/laloi/game/${roomCode}/play`;
            } else {

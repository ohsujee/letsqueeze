...
export function useDailyGame(gameId, { forceDate } = {}) {
  const todayDate = forceDate || new Date().toLocaleDateString('en-CA', { timeZone: 'Europe/Paris' });
  ...
  const saveProgress = useCallback((guesses, attempts, feedbacks = []) => {
    if (!storageKey) return;
  ...
  /** Écrit uniquement dans le leaderboard Firebase (sans toucher streak/stats) */
  const writeLeaderboard = useCallback(async ({ score, attempts, solved, timeMs }) => {
    if (!user) return;
    try {
      ...
      console.warn('[useDailyGame] writeLeaderboard error:', e);
    }
  }, [user, displayName, gameId, todayDate]);

  /** Marque la partie comme terminée + mise à jour streak/stats + Firebase */
  const completeGame = useCallback(async ({ solved, attempts, timeMs, score, revealedWord = null, skipLeaderboard = false }) => {
    if (!storageKey) return;
    const now = Date.now();
    const current = storage.get(storageKey) || {};
    const completed = {
    ...
    // Comptabilise la partie pour l'ATT prompt et les pubs
  ...
  return {
    ...
    progress: todayData ? {
      ...
      feedbacks: todayData.feedbacks || [],
      score: todayData.score,
      timeMs: todayData.timeMs,
      solved: todayData.solved,
      revealedWord: todayData.revealedWord || null,
    } : null,
    todayDate,
    startGame,
    saveProgress,

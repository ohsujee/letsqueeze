...
export function useDailyGame(gameId, { forceDate } = {}) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      try {
        ...
        const [gameSnap, streakSnap, statsSnap] = await Promise.race([
          Promise.all([
      ...
      } catch { /* non-bloquant - fallback localStorage */ }
  ...
  const completeGame = useCallback(async ({ solved, attempts, timeMs, score, revealedWord = null }) => {
    if (!storageKey) return;
    const now = Date.now();
    const current = storage.get(storageKey) || {};
    const completed = {

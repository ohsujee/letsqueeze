...
export function useDailyGame(gameId, { forceDate } = {}) {
  ...
  // Écouter l'auth + résoudre le pseudo + restaurer streak/stats depuis Firebase si localStorage vide
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        // Restaurer streak + stats depuis Firebase si localStorage vide
        try {
          const savedStreak = storage.get(streakKey);
          const savedStats = storage.get(statsKey);
          if (!savedStreak || !savedStats) {
            const [streakSnap, statsSnap] = await Promise.all([
              get(ref(db, `users/${u.uid}/game_data/${gameId}/streak`)),
              get(ref(db, `users/${u.uid}/game_data/${gameId}/stats`)),
            ]);
            if (!savedStreak && streakSnap.val()) {
              storage.set(streakKey, streakSnap.val());
              setStreak(streakSnap.val());
            }
            if (!savedStats && statsSnap.val()) {
              storage.set(statsKey, statsSnap.val());
              setStats(statsSnap.val());
            }
          }
        } catch { /* ignore, données locales suffisent */ }
      } else {
        setDisplayName(null);
      }
    });
    return () => unsub();
  }, [gameId, streakKey, statsKey]);
  ...
  const completeGame = useCallback(async ({ solved, attempts, timeMs, score }) => {
    ...
    // Sauvegarde streak + stats sur Firebase (restauration si données supprimées)
    if (user) {
      try {
        await Promise.all([
    ...
    // Écriture Firebase leaderboard (si connecté, anonyme inclus — victoire ET défaite)
    if (user) {
      try {

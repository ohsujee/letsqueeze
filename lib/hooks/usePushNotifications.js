  ...
  useEffect(() => {
    ...
    const saveToken = async (token, user) => {
      ...
      try {
        ...
        // Enregistrer version + plateforme pour stats
        const { App } = await import('@capacitor/app');
        const info = await App.getInfo();
        await set(ref(db, `users/${user.uid}/appVersion`), info.version);
        await set(ref(db, `users/${user.uid}/platform`), Capacitor.getPlatform());
      } catch {}
    };
    ...
    const init = async () => {
      ...
      // Vider le badge + les notifs du centre quand l'app passe au premier plan
      const { App } = await import('@capacitor/app');
      ...
      listenersRef.current.push(stateHandle);

...
export function useAlibiGroupRotation({
  ...
  const generateRoundRotation = useCallback((groupIdList, questionsPerGroup) => {
    ...
    // Pour chaque cycle, chaque groupe interroge le groupe suivant
    // On alterne le groupe qui commence pour équilibrer
    for (let cycle = 0; cycle < questionsPerGroup; cycle++) {
      for (let i = 0; i < numGroups; i++) {
        // Alterner quel groupe commence à chaque cycle
        const inspectorIdx = (i + cycle) % numGroups;
        const accusedIdx = (inspectorIdx + 1) % numGroups;

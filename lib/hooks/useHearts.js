...
export function useHearts({ isPro = false } = {}) {
  ...
  const rechargeHearts = useCallback(async () => {
    ...
    try {
      ...
      // Les rewarded ads ne peuvent pas être skippées → tout échec est technique.
      // On recharge quand même. On distingue juste si la pub a vraiment joué (pour le skip interstitiel).

...
export function useHearts({ isPro = false, uid = null } = {}) {
  ...
  // Résoudre l'uid : prop fourni OU détection auto depuis Firebase Auth.
  // Les pages n'ont pas besoin de passer uid explicitement.
  const [resolvedUid, setResolvedUid] = useState(uid);
  useEffect(() => {
    if (uid !== null) {
      setResolvedUid(uid);
      uidRef.current = uid;
    ...
    const unsub = onAuthStateChanged(auth, (user) => {
      const autoUid = user?.uid ?? null;
      setResolvedUid(autoUid);
      uidRef.current = autoUid;
    });
    return unsub;
  }, [uid]);
  ...
  useEffect(() => {
    ...
    const heartsRef = ref(db, `users/${resolvedUid}/hearts`);
  ...
  const canRecharge = !isPro && heartsRemaining < MAX_HEARTS;

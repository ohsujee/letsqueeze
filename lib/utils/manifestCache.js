 ...
 * Automatically calculates questionCount from actual database files
 */
export async function getQuizManifest() {
  ...
  try {
    ...
    const countPromises = allThemes.map(theme =>
      fetchQuestionCount(theme.id).then(count => ({ id: theme.id, count }))
    );
    ...
    // Update questionCount for each theme
    categories.forEach(category => {
      (category.themes || []).forEach(theme => {
        if (countMap[theme.id] !== undefined) {
          theme.questionCount = countMap[theme.id];
        }
      });
    });

...
async function fetchQuestionCount(databaseId) {
  ...
  try {
    const res = await fetch(`/data/quiz/${databaseId}.json`);
    if (!res.ok) {
...
export async function getQuizManifest() {
  ...
  try {
    const res = await fetch('/data/quiz/manifest.json');
    const data = await res.json();
    const categories = data.categories || [];

    // Enrichir avec les vrais counts depuis les fichiers JSON
    for (const category of categories) {
      for (const theme of category.themes) {
        if (!theme.comingSoon) {
          theme.questionCount = await fetchQuestionCount(theme.id);
        }
      }
    }

...
export const signInWithApple = async () => {
  try {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      // With skipNativeAuth: false (default), the plugin handles Firebase sign-in automatically
      // We just need to call signInWithApple() and the user will be signed in
      const result = await FirebaseAuthentication.signInWithApple();

      // The plugin already signed in the user natively
      // result.user contains the Firebase user
      console.log('[Auth] Apple Sign-In completed natively');

      // Note: Anonymous account linking not supported with native auth
      // User will get a new account (their anonymous stats won't transfer)
      if (isAnonymous) {
        console.log('[Auth] Warning: Anonymous account cannot be linked with native Apple Sign-In');
      }

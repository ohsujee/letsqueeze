...
import {
  getDatabase, ref, set, get, onValue, update, remove, runTransaction, serverTimestamp, increment
...
export const signInWithGoogle = async () => {
  try {
    ...
    // Fallback : si Credential Manager échoue, retry avec l'ancien GoogleSignInClient
    const isCredentialManagerError =
      msg.includes('No credentials available') ||
      msg.includes("doesn't support credential manager") ||
      msg.includes('GetCredentialProviderConfigurationException') ||
      msg.includes('NoCredentialException');

    if (Capacitor.getPlatform() === 'android' && isCredentialManagerError) {
      try {
        const { FirebaseAuthentication } = await import('@capacitor-firebase/authentication');
        const currentUser = auth.currentUser;
        const isAnonymous = currentUser?.isAnonymous;
        ...
        // Log le fallback échoué aussi
        try {
          set(ref(db, `debug_logs/${Date.now()}`), {
            fn: 'signInWithGoogle_fallback',
        ...
        try {
          ...
          await FirebaseCrashlytics.recordException({ message: `signInWithGoogle fallback failed: ${fallbackError.message}` });
        } catch {}
        throw fallbackError;
      }
    }
    ...
    try {
      const { FirebaseCrashlytics } = await import('@capacitor-firebase/crashlytics');
      await FirebaseCrashlytics.recordException({ message: `signInWithGoogle failed: ${msg}` });
    } catch {}
...
export const signInWithApple = async () => {
  try {
    ...
    try {
      const { FirebaseCrashlytics } = await import('@capacitor-firebase/crashlytics');
      await FirebaseCrashlytics.recordException({ message: `signInWithApple failed: ${error.message}` });
    } catch {}
...
export {
  ref, set, get, onValue, update, remove, runTransaction, serverTimestamp, increment,

...
function saveTokens(tokens) {
  console.log('[Spotify Auth] Saving tokens:', {
  ...
  if (tokens.refresh_token) {
    ...
    console.log('[Spotify Auth] Refresh token saved');
  } else {
    console.warn('[Spotify Auth] No refresh token in response!');
  }
}
...
export async function getAccessToken() {
  ...
  console.log('[Spotify Auth] getAccessToken:', {
  ...
  if (!accessToken) {
    console.warn('[Spotify Auth] No access token found');
    return null;
  }

  // Si le token expire dans moins de 5 minutes, le rafraîchir
  if (Date.now() > expiresAt - 5 * 60 * 1000) {
    console.log('[Spotify Auth] Token expired or expiring soon, need refresh');

    if (!refreshToken) {
      console.error('[Spotify Auth] Cannot refresh: no refresh token available');
      // Return the existing token anyway - it might still work
      console.log('[Spotify Auth] Returning existing token (might be expired)');
      return accessToken;
    }
...
function getRefreshToken() {
  ...
  console.log('[Spotify Auth] getRefreshToken:', !!token);
  return token;
}

...
export async function initializePlayer({ onReady, onStateChange, onError } = {}) {
  console.log('[Spotify Player] initializePlayer called:', {
    hasPlayer: !!player,
    isReady,
    deviceId
  });
  ...
  if (player && isReady && deviceId) {
    console.log('[Spotify Player] Reusing existing player connection');
    onReady?.(deviceId);
    return deviceId;
  }
  ...
  if (player && !isReady) {
    console.log('[Spotify Player] Player exists but not ready, attempting reconnect...');
    try {
      const success = await player.connect();
      console.log('[Spotify Player] Reconnect result:', success);
      if (success) {
        // Wait for ready event
        return new Promise((resolve, reject) => {
          const timeout = setTimeout(() => {
            console.log('[Spotify Player] Reconnection timeout');
            reject(new Error('Reconnection timeout'));
          }, 10000);

          onReadyCallback = (id) => {
            clearTimeout(timeout);
            console.log('[Spotify Player] Reconnected and ready:', id);
            onReady?.(id);
            resolve(id);
          };
      ...
      console.log('[Spotify Player] Reconnect failed, creating new player:', e);
    }
  }

  console.log('[Spotify Player] Creating new player...');
  ...
  try {
      ...
      keepAliveInterval = setInterval(async () => {
        if (player && isReady) {
          try {
            ...
            console.log('[Spotify Player] Keep-alive ping');
          } catch (e) {
            console.log('[Spotify Player] Keep-alive failed, connection may be lost');
          }
        }
    ...
    player.addListener('not_ready', ({ device_id }) => {
      ...
      console.warn('[Spotify Player] Stack trace:', new Error().stack);
      isReady = false;
      // Try to reconnect automatically
      setTimeout(async () => {
        if (!isReady && player) {
          console.log('[Spotify Player] Attempting auto-reconnect...');
          try {
...
/**
 * Précharge une track silencieusement (volume 0, puis pause)
 * Permet d'éviter le délai de chargement lors du premier play
 * @param {string} trackUri - URI Spotify (spotify:track:xxx)
 */
let preloadAbortController = null;

// Listener for preload state changes
let preloadStateResolver = null;

export async function preloadTrack(trackUri) {
  const accessToken = await getAccessToken();

  console.log('[Spotify Player] preloadTrack called:', {
  ...
  if (!accessToken || !deviceId || !player || !isReady) {
    console.log('[Spotify Player] Cannot preload - player not ready, skipping');
    return;
  }
  ...
  try {
    // Set volume to 0 BEFORE anything else
    ...
    const playbackStarted = new Promise((resolve) => {
      preloadStateResolver = resolve;
      // Timeout after 3 seconds
      setTimeout(() => {
        preloadStateResolver = null;
        resolve();
      }, 3000);
    });

    // Start playing at position 0 (this triggers buffering)
    ...
    if (!response.ok && response.status !== 204) {
      console.log('[Spotify Player] Preload play request failed:', response.status);
      preloadStateResolver = null;
      return;
    }
    ...
    // Wait for playback to actually start (confirmed by state change)
    await playbackStarted;
    preloadStateResolver = null;

    if (signal.aborted) return;

    // Now pause - playback has confirmed started so pause should work
    console.log('[Spotify Player] Playback confirmed, now pausing...');

    // Pause via SDK
    await player.pause();

    // Double-check with API pause
    ...
    // Triple-check: verify it's actually paused
    await new Promise(resolve => setTimeout(resolve, 100));
    const state = await player.getCurrentState();
    if (state && !state.paused) {
      console.log('[Spotify Player] Still playing after pause, forcing pause again...');
      await player.pause();
    }

    console.log('[Spotify Player] Track preloaded and paused:', trackUri);
  } catch (e) {
    if (e.name === 'AbortError') return;
    console.log('[Spotify Player] Preload failed:', e.message);
    preloadStateResolver = null;

    // Try to pause anyway in case of error
    try {
      await player?.pause();
    } catch (_) {}
  }
...
// Call this from player state change listener
export function notifyPreloadStateChange(state) {
  if (preloadStateResolver && state && !state.paused) {
    console.log('[Spotify Player] Preload playback started, resolving...');
    preloadStateResolver();
    preloadStateResolver = null;
  }
}
...
export async function playTrack(trackUri, positionMs = 0) {
  ...
  console.log('[Spotify Player] playTrack called:', {
    hasToken: !!accessToken,
    deviceId,
    hasPlayer: !!player,
    isReady
  ...
  // If player exists but not ready, try to reconnect
  if (player && (!isReady || !deviceId)) {
    console.log('[Spotify Player] Player exists but not ready, reconnecting...');
    try {
      await player.connect();
      // Wait for ready
  ...
  if (!response.ok && response.status !== 204) {
    ...
    if (response.status === 404) {
      console.log('[Spotify Player] Device not found, attempting to reconnect...');
      isReady = false;
...
export async function pause() {
  if (player) {
    try {
      ...
      console.log('[Spotify Player] Pause skipped - no track loaded');
    }
  }
}
...
export async function resume() {
  if (player) {
    try {
      ...
      console.log('[Spotify Player] Resume skipped - no track loaded');
    }
  }
}

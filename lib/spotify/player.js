      ...
      keepAliveInterval = setInterval(async () => {
        if (player && isReady) {
          try {
            ...
            if (!state) {
              console.log('[Spotify Player] Keep-alive: no state, reconnecting...');
              isReady = false;
              await player.connect().catch(() => {});
            }
          } catch (e) {
            console.log('[Spotify Player] Keep-alive failed, attempting reconnect...');
            isReady = false;
            try {
              await player.connect();
            } catch (reconnectErr) {
              console.log('[Spotify Player] Reconnect failed:', reconnectErr);
            }
          }
        }
      }, 15000); // Ping every 15 seconds (was 30s)
...
export async function preloadTrack(trackUri) {
  ...
  console.log('[Spotify Player] preloadTrack called:', { sessionId, trackUri: trackUri?.slice(-10) });

  if (!accessToken || !deviceId || !player || !isReady) {
    console.log('[Spotify Player] Cannot preload - player not ready');
  ...
  try {
    ...
    if (!playResponse.ok && playResponse.status !== 204) {
      console.log('[Spotify Player] Preload play failed:', playResponse.status);
    ...
    console.log('[Spotify Player] Pausing after preload...');
    ...
    while (attempts < 5) {
      ...
      console.log('[Spotify Player] Still playing, attempt', attempts + 1);
      await player.pause().catch(() => {});
    ...
    console.log('[Spotify Player] Track preloaded and ready:', trackUri?.slice(-20));
    return { success: true };

 ...
 * Note: Le package @capgo/capacitor-social-login n'est disponible que dans les builds Capacitor
 */
const initSocialLogin = async () => {
  ...
  if (!socialLoginInstance) {
    try {
      // Dynamic import avec webpackIgnore pour éviter l'erreur de build web
      const module = await import(/* webpackIgnore: true */ '@capgo/capacitor-social-login');
      const SocialLogin = module.SocialLogin;
      ...
      console.warn('SocialLogin plugin not available:', error.message);
      return null;
    }
  }
 ...
 * Masqué sur Android (pas utile pour les utilisateurs Android)
 * @returns {boolean}
 */
export const shouldShowAppleSignIn = () => {
  const platform = getPlatform();

  // Masquer sur Android natif (les utilisateurs Android n'ont pas de compte Apple)
  ...
  // Masquer sur navigateurs Android
  if (isAndroidDevice()) return false;

  // Sur Web desktop, afficher pour les utilisateurs Mac/iPhone
  return true;
};
...
const signInWithGoogleNative = async () => {
  try {
    console.log('[GoogleNative] Initializing SocialLogin...');
    const SocialLogin = await initSocialLogin();

    if (!SocialLogin) {
      const err = new Error('[GoogleNative] SocialLogin plugin unavailable (initSocialLogin returned null)');
      err.code = 'plugin/unavailable';
      throw err;
    }

    console.log('[GoogleNative] Calling SocialLogin.login...');
    ...
    console.log('[GoogleNative] SocialLogin.login result:', JSON.stringify(result));

    if (!result?.result?.idToken) {
      const err = new Error(`[GoogleNative] No idToken in result: ${JSON.stringify(result)}`);
      err.code = 'google/no-id-token';
      throw err;
    }

    // Créer le credential Firebase avec le token
    const credential = GoogleAuthProvider.credential(result.result.idToken);
    console.log('[GoogleNative] signInWithCredential...');
    return signInWithCredential(auth, credential);
  } catch (error) {
    console.error('[GoogleNative] Error:', error?.code, error?.message, error);
    // Enrichir l'erreur avec plus de contexte
    if (!error.code) error.code = 'google/unknown-native';
    throw error;
  }
};

...
export const ALIBI_GROUP_CONFIG = {
  ...
  DEFAULT_COLORS: ['#E84466', '#1EAEE0', '#30C968', '#DDB830'],
  QUESTIONS_PER_GROUP: {
  ...
  },
  {
    ...
    createExtra: ({ gameMasterMode }) => {
      if (gameMasterMode === 'party') {
        const groups = {};
        for (let i = 0; i < ALIBI_GROUP_CONFIG.MIN_GROUPS; i++) {
          groups[`group${i + 1}`] = {
            id: `group${i + 1}`,
            name: ALIBI_GROUP_CONFIG.DEFAULT_NAMES[i],
            color: ALIBI_GROUP_CONFIG.DEFAULT_COLORS[i],
 ...
 * @param {string} params.audioMode - 'single' or 'all' (for deeztest)
 ...
 */
export async function createRoom({ gameId, code, hostUid, hostName, gameMasterMode = 'gamemaster', audioMode = 'single', db, ref, set }) {
  ...
  // Write meta FIRST — other writes depend on meta/hostUid for Firebase rules
  await set(ref(db, `${prefix}/${code}/meta`), roomType.createMeta(params));

  // Now write state + extra in parallel (meta exists, rules will pass)

...
/**
 * Get all room prefixes for checking code uniqueness
 */
export function getAllRoomPrefixes() {
  return ROOM_TYPES.map(rt => rt.prefix);
}
...
/**
 * Get room type config by prefix
 */
export function getRoomTypeByPrefix(prefix) {
  return ROOM_TYPES.find(rt => rt.prefix === prefix);
}
